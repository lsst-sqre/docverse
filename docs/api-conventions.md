...
- **`PATCH` for partial updates with JSON-Merge-Patch semantics.**
  ...
  **Merge-patch semantics** (house style): omitted fields are left
  ...
  `{"enabled": false}` leaves `ltd_base_url` and the scope fields
  unchanged, while providing `project_slugs` (or any of
  `project_slug_patterns`, `exclude_project_slugs`,
  `exclude_project_slug_patterns`) replaces that entire list.
  ...
  **Explicit `null` is rejected, not honoured.** RFC 7386 merge-patch gives
  ...
  keeper-sync config's `enabled` / `ltd_base_url` / `project_slugs` and
  its three scope fields), so
