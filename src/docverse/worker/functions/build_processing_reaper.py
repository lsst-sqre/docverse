...
# Window before a queued ``build_processing`` row with no
...
async def build_processing_reaper(ctx: dict[str, Any]) -> str:
    ...
    1. Silent rows
       (:meth:`QueueJobStore.fail_silent_build_processing_jobs`) —
       ``status='in_progress'`` past
       ``config.build_processing_reaper_threshold_seconds`` (default
       8 h, env-overridable for fast verification in non-prod).
    2. Orphan rows
       (:meth:`QueueJobStore.fail_orphaned_build_processing_jobs`) —
    ...
    """
    logger = structlog.get_logger("docverse.worker.build_processing_reaper")
    threshold = timedelta(
        seconds=config.build_processing_reaper_threshold_seconds
    )
    ...
    async for session in db_session_dependency():
        ...
        async with session.begin():
            silent = await queue_job_store.fail_silent_build_processing_jobs(
                idle_after=threshold
            )
            orphan = await queue_job_store.fail_orphaned_build_processing_jobs(
        ...
        if reaped_count:
            logger.warning(
                "Reaped stuck build_processing queue jobs",
            ...
            logger.debug("No stuck build_processing queue jobs to reap")
        return "completed"
