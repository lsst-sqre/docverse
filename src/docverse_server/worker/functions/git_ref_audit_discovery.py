...
from docverse.models import GitRefAuditRunStatus, JobKind
from docverse_server.config import config
from docverse_server.domain.git_ref_audit_run import GitRefAuditRun
...
async def git_ref_audit_discovery(ctx: dict[str, Any]) -> str:
    ...
    logger = structlog.get_logger(
        "docverse_server.worker.git_ref_audit_discovery"
    )
...
async def _create_run_with_children(
    *,
    ...
    Mirrors :func:`docverse_server.worker.functions.lifecycle_eval_dispatcher
