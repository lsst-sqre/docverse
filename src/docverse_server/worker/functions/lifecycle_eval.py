...
:func:`docverse_server.services.lifecycle.evaluator.evaluate_lifecycle`
...
then calls
:func:`docverse_server.services.lifecycle_finalisation.maybe_finalise_lifecycle_run`
...
async def lifecycle_eval(ctx: dict[str, Any], payload: dict[str, Any]) -> str:
    ...
    logger = structlog.get_logger(
        "docverse_server.worker.lifecycle_eval"
    ).bind(org=org_slug, lifecycle_eval_run_id=run_id)
    ...
    async for session in db_session_dependency():
        ...
        async with session.begin():
            ...
            # behalf, so re-running the evaluation would double-count.
