"""Tests for the ``dashboard_build_reaper`` cron worker function.

Mirrors :mod:`tests.worker.lifecycle_reaper_test` for the
``dashboard_build`` subsystem. The reaper is the cron-driven backstop
for the case where arq itself loses a ``dashboard_build`` job — a
...
``failed`` and sweeps orphan queued rows so the per-project mutex
``idx_queue_jobs_dashboard_build_active_uq`` releases — eliminating
the user-visible 409 on ``POST /dashboard/rebuild``.
"""
...
async def _seed_silent_row(
    ...
    *,
    ...
    """Insert a ``kind='dashboard_build'`` row stuck in ``in_progress``."""
    row = SqlQueueJob(
...
async def _seed_orphan_row(
    ...
    *,
    ...
    """Insert a ``kind='dashboard_build'`` orphan (queued, no backend id)."""
    row = SqlQueueJob(
        public_id=validate_base32_id(generate_base32_id()),
        backend_job_id=None,
        kind=JobKind.dashboard_build.value,
...
async def test_reaper_fails_stuck_in_progress(
    ...
    async with db_session.begin():
        org_id = await _seed_org(db_session, slug="dbr-1")
    ...
    try:
        result = await dashboard_build_reaper(ctx)
    finally:
...
async def test_reaper_sweeps_orphan_queued_row(
    ...
    ever pick up. Without the orphan sweep the per-project mutex
    ``idx_queue_jobs_dashboard_build_active_uq`` keeps observing the
    stale row and the operator's next ``POST /dashboard/rebuild`` is
    blocked with a 409.
    """
    async with db_session.begin():
        org_id = await _seed_org(db_session, slug="dbr-2")
...
async def test_reaper_no_op_logs_debug_not_warning(
    ...
    logs every 15 minutes when the reaper finds zero candidate rows
    — PRD #367 user story 16.
    """
    async with db_session.begin():
        org_id = await _seed_org(db_session, slug="dbr-3")
    ...
    reaper_events = [
        ...
        if "dashboard_build" in str(entry.get("event", ""))
    ]
    warnings = [
...
async def test_reaper_warning_includes_count_and_public_ids(
    ...
    """
    async with db_session.begin():
        org_id = await _seed_org(db_session, slug="dbr-4")
    ...
    warnings = [
        ...
        and "dashboard_build" in str(entry.get("event", ""))
    ]
...
async def test_reaper_threshold_is_configurable(
    ...
    ``DOCVERSE_DASHBOARD_BUILD_REAPER_THRESHOLD_SECONDS`` to a small
    value so a deliberately-wedged job surfaces in seconds rather
    than the production-default 30 minutes. The reaper must observe
    the configured value at invocation time rather than a baked-in
    default.
    """
    monkeypatch.setattr(
        runtime_config, "dashboard_build_reaper_threshold_seconds", 60
    )

    async with db_session.begin():
        org_id = await _seed_org(db_session, slug="dbr-5")
        stuck_id = await _seed_silent_row(
            db_session,
            org_id=org_id,
...
async def test_reaper_isolates_other_main_pool_kinds(
    ...
    """Cross-kind isolation: only ``dashboard_build`` rows are reaped.

    Seeds ``in_progress`` rows of every other main-pool kind plus
    ``lifecycle_eval`` past the dashboard_build threshold. Only the
    ``dashboard_build`` row should move to ``failed`` — the rest
    must remain ``in_progress``.
    """
    async with db_session.begin():
        org_id = await _seed_org(db_session, slug="dbr-iso")
        ...
        for idx, kind in enumerate(other_kinds):
            other_row = SqlQueueJob(
                ...
                status=JobStatus.in_progress.value,
                org_id=org_id,
                date_started=(datetime.now(tz=UTC) - timedelta(minutes=60)),
            )
