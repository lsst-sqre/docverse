"""Tests for the ``build_processing_reaper`` cron worker function.

Mirrors :mod:`tests.worker.publish_edition_reaper_test` for the
``build_processing`` subsystem. The reaper is the cron-driven backstop
for the case where arq itself loses a ``build_processing`` job — a
...
``failed`` and sweeps orphan queued rows so an uploaded build does
not stay unregistered indefinitely after a worker crash.
"""
...
async def _seed_silent_row(
    ...
    *,
    ...
    """Insert a ``kind='build_processing'`` row stuck in ``in_progress``."""
    row = SqlQueueJob(
...
async def _seed_orphan_row(
    ...
    *,
    ...
    """Insert a ``kind='build_processing'`` orphan (queued, no backend id)."""
    row = SqlQueueJob(
        public_id=validate_base32_id(generate_base32_id()),
        backend_job_id=None,
        kind=JobKind.build_processing.value,
...
async def test_reaper_fails_stuck_in_progress(
    ...
    async with db_session.begin():
        org_id = await _seed_org(db_session, slug="bpr-1")
...
async def test_reaper_sweeps_orphan_queued_row(
    ...
    ever pick up. Without the orphan sweep the uploaded build is
    never registered and the project never sees progress on its new
    release.
    """
    async with db_session.begin():
        org_id = await _seed_org(db_session, slug="bpr-2")
...
async def test_reaper_no_op_logs_debug_not_warning(
    ...
    """
    async with db_session.begin():
        org_id = await _seed_org(db_session, slug="bpr-3")
...
async def test_reaper_warning_includes_count_and_public_ids(
    ...
    """
    async with db_session.begin():
        org_id = await _seed_org(db_session, slug="bpr-4")
    ...
    warnings = [
        ...
        and "build_processing" in str(entry.get("event", ""))
    ]
...
async def test_reaper_threshold_is_configurable(
    ...
    ``DOCVERSE_BUILD_PROCESSING_REAPER_THRESHOLD_SECONDS`` to a small
    value so a deliberately-wedged job surfaces in seconds rather
    than the production-default 8 hours. The reaper must observe the
    configured value at invocation time rather than a baked-in
    default.
    """
    monkeypatch.setattr(
        runtime_config, "build_processing_reaper_threshold_seconds", 60
    )

    async with db_session.begin():
        org_id = await _seed_org(db_session, slug="bpr-5")
        stuck_id = await _seed_silent_row(
            db_session,
            org_id=org_id,
...
async def test_reaper_isolates_other_main_pool_kinds(
    ...
    """Cross-kind isolation: only ``build_processing`` rows are reaped.

    Seeds ``in_progress`` rows of every other main-pool kind plus
    ``lifecycle_eval`` past the build_processing threshold. Only the
    ``build_processing`` row should move to ``failed`` — the rest
    must remain ``in_progress``.
    """
    async with db_session.begin():
        org_id = await _seed_org(db_session, slug="bpr-iso")
        ...
        for idx, kind in enumerate(other_kinds):
            other_row = SqlQueueJob(
                ...
                status=JobStatus.in_progress.value,
                org_id=org_id,
                date_started=(datetime.now(tz=UTC) - timedelta(minutes=600)),
            )
