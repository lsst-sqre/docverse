"""Tests for the ``dashboard_sync_reaper`` cron worker function.

Mirrors :mod:`tests.worker.build_processing_reaper_test` for the
``dashboard_sync`` subsystem. The reaper is the cron-driven backstop
for the case where arq itself loses a ``dashboard_sync`` job — a
...
``failed`` and sweeps orphan queued rows so a binding's
``last_sync_queue_job`` does not show a permanently in-progress sync
...
async def _seed_silent_row(
    ...
    *,
    ...
    """Insert a ``kind='dashboard_sync'`` row stuck in ``in_progress``."""
    row = SqlQueueJob(
...
async def _seed_orphan_row(
    ...
    *,
    ...
    """Insert a ``kind='dashboard_sync'`` orphan (queued, no backend id)."""
    row = SqlQueueJob(
        public_id=validate_base32_id(generate_base32_id()),
        backend_job_id=None,
        kind=JobKind.dashboard_sync.value,
...
async def test_reaper_fails_stuck_in_progress(
    ...
    async with db_session.begin():
        org_id = await _seed_org(db_session, slug="dsr-1")
        stuck_id = await _seed_silent_row(
            db_session,
            org_id=org_id,
            backend_job_id="arq-stuck-1",
            started_minutes_ago=480,
            project_id=101,
        )
    ...
    try:
        result = await dashboard_sync_reaper(ctx)
    finally:
...
async def test_reaper_sweeps_orphan_queued_row(
    ...
    ever pick up. Without the orphan sweep the binding's
    ``last_sync_queue_job`` shows a permanently queued sync.
    """
    async with db_session.begin():
        org_id = await _seed_org(db_session, slug="dsr-2")
    ...
    ctx = _make_ctx(http_client)
    try:
        await dashboard_sync_reaper(ctx)
    finally:
        await ctx["http_client"].aclose()
...
async def test_reaper_no_op_logs_debug_not_warning(
    ...
    """
    async with db_session.begin():
        org_id = await _seed_org(db_session, slug="dsr-3")
    ...
    reaper_events = [
        ...
        if "dashboard_sync" in str(entry.get("event", ""))
    ]
    warnings = [
...
async def test_reaper_warning_includes_count_and_public_ids(
    ...
    """
    async with db_session.begin():
        org_id = await _seed_org(db_session, slug="dsr-4")
    ...
    warnings = [
        ...
        and "dashboard_sync" in str(entry.get("event", ""))
    ]
...
async def test_reaper_threshold_is_configurable(
    ...
    ``DOCVERSE_DASHBOARD_SYNC_REAPER_THRESHOLD_SECONDS`` to a small
    value so a deliberately-wedged job surfaces in seconds rather
    than the production-default 6 hours. The reaper must observe
    the configured value at invocation time rather than a baked-in
    default.
    """
    monkeypatch.setattr(
        runtime_config, "dashboard_sync_reaper_threshold_seconds", 60
    )

    async with db_session.begin():
        org_id = await _seed_org(db_session, slug="dsr-5")
        stuck_id = await _seed_silent_row(
            db_session,
            org_id=org_id,
...
async def test_reaper_isolates_other_main_pool_kinds(
    ...
    """Cross-kind isolation: only ``dashboard_sync`` rows are reaped.

    Seeds ``in_progress`` rows of every other main-pool kind plus
    ``lifecycle_eval`` past the dashboard_sync threshold. Only the
    ``dashboard_sync`` row should move to ``failed`` — the rest
    must remain ``in_progress``.
    """
    async with db_session.begin():
        org_id = await _seed_org(db_session, slug="dsr-iso")
        ...
        for idx, kind in enumerate(other_kinds):
            other_row = SqlQueueJob(
                ...
                status=JobStatus.in_progress.value,
                org_id=org_id,
                date_started=(datetime.now(tz=UTC) - timedelta(minutes=480)),
            )
