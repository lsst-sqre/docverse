...
async def _seed_content(session: AsyncSession) -> int:
    logger = structlog.get_logger("test")
    store = DashboardTemplateContentStore(session=session, logger=logger)
...
@pytest.mark.asyncio
async def test_db_template_source_satisfies_protocol(
    db_session: AsyncSession,
) -> None:
    async with db_session.begin():
        content_id = await _seed_content(db_session)
        await db_session.commit()
    source = DbTemplateSource(content_id=content_id, session=db_session)
    assert isinstance(source, TemplateSource)


@pytest.mark.asyncio
async def test_db_template_source_loads_config_after_preload(
    db_session: AsyncSession,
) -> None:
    async with db_session.begin():
        content_id = await _seed_content(db_session)
...
@pytest.mark.asyncio
async def test_db_template_source_caches_parsed_config(
    db_session: AsyncSession,
) -> None:
    async with db_session.begin():
        content_id = await _seed_content(db_session)
...
@pytest.mark.asyncio
async def test_db_template_source_reads_text_template(
    db_session: AsyncSession,
) -> None:
    async with db_session.begin():
        content_id = await _seed_content(db_session)
...
@pytest.mark.asyncio
async def test_db_template_source_reads_binary_asset(
    db_session: AsyncSession,
) -> None:
    async with db_session.begin():
        content_id = await _seed_content(db_session)
...
@pytest.mark.asyncio
async def test_db_template_source_missing_file_raises_filenotfound(
    db_session: AsyncSession,
) -> None:
    async with db_session.begin():
        content_id = await _seed_content(db_session)
...
@pytest.mark.asyncio
async def test_db_template_source_unknown_content_raises(
    db_session: AsyncSession,
) -> None:
    source = DbTemplateSource(content_id=999_999, session=db_session)
    with pytest.raises(LookupError):
        await source.preload()


@pytest.mark.asyncio
async def test_db_template_source_methods_require_preload(
    db_session: AsyncSession,
) -> None:
    async with db_session.begin():
        content_id = await _seed_content(db_session)
